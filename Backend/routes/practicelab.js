...
// Backward-compatible alias for older frontend builds that still call /api/ptl/machines
router.get("/api/ptl/machines", (req, res) => {
  const track = normalizeText(req.query.track);
  const level = normalizeText(req.query.level || req.query.difficulty);
  const search = normalizeText(req.query.search);
  ...
  return res.json({
    ...
    machines: labs.map((lab) => ({
      ...
      description: lab.description,
      taskCount: lab.tasks.length,
      tags: [normalizeText(lab.track), normalizeText(lab.level), "commands", "learning"],
    })),
  });
});
